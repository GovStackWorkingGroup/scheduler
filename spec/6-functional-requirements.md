...
6.1.5 It MUST be possible to find details of all events in a chosen date-time range, filtered by a given category, having specific resources and still open for subscription.
...
6.1.7. The Scheduler should send notifications to all resources and subscribers when an associated event is deleted.
...
6.2.5 The Scheduler MUST generate a unique id for each new resource registered, which is independent of which events the resources participates in. This is needed to uniquely find and reuse details of the resource for various events as needed.
...
6.2.8. The Scheduler MUST convey resource id to a specific resource upon registration.

6.2.9. The Scheduler MUST delete all affiliations and future appointments of a resource that is deleted. In a real-world implementation, a resource may be just deactivated instead of actually deleted.

6.2.10 If a Resource is deleted, the corresponding resource and Organizers of all affiliated entities must be notified about the deletion. In a real-world implementation, a resource may be de-affiliated in a particular entity, instead of deletion from the resource registry.
...
6.3.5 A Resource MAY be booked into multiple entities with different affiliation IDs as long as they have non-overlapping working hours and days for that resource. However, the Resource ID shall be the same in all such affiliations.
...
6.4.4 Contact details of the subscriber (email/sms/webhook/etc. along with default option) MUST be registered along with the subscriber.

6.4.5 The Scheduler MUST generate a unique id for each new subscriber registered, which is independent of which events the subscribers participates in. This is needed to uniquely find and reuse details of the subscriber for various events as needed.

6.4.6 The Scheduler MUST convey subscriber ID to a specific subscriber upon registration.

6.4.7 If a subscriber is deleted the Scheduler MUST delete all future appointments of the subscriber. In a real-world implementation, a subscriber may be just deactivated instead of actually deleted.

### 6.5. Alert Message Management

6.5.1 The Scheduler MUST enable an event organizer to create/update/delete/modify/list Alert message templates that can be used to communicate with participants of events. ( e.g. appointment reminders to various patients, triggering a periodic transfer of salaries, etc.).
...
The Scheduler MUST generate a unique id for each new message registered, which is independent of which events the message is used in. This is needed to uniquely find and reuse details of the message for various events as needed.

6.5.4 In the current scope only textual messages are considered as content of the templates.
...
6.5.5 If an alert message template is deleted, the Scheduler must delete the bindings to any future events and notify the organizer of the entity about the deletion.

### 6.6. Alert Schedule Management <a href="#_heading-h.3j2qqm3" id="_heading-h.3j2qqm3"></a>

6.6.1 The Scheduler MUST enable an organizer to create/update/delete/search and list alert\_schedules for sending alert messages in association with a specific event. A single alert schedule specifies details of only one epoch of sending a specific message to a specific category of participants of a single event.
...
6.6.4 The Scheduler MUST NOT allow the reuse of an alert scheduler either in the same or another event

6.6.5 All the details about the alert schedules MUST be stored in a common internal repository so that the alerts may be sent to multiple targets in high throughput environments

6.6.6 The Scheduler MUST enable an organizer to configure a specific date-time before/during/after the event.

6.6.7 The scheduler must enable the Organizer to choose if the alert should target either all subscribers or all resources or both types of participants in a specific event.
...
6.6.9 The alert messages MUST be composed with associated supplementary information (e.g. who is sending, for which event, what category of message, etc.) along with corresponding contact information and routed to appropriate channels (Information Mediator Building Block/Messaging Building Block/PubSub)
...
6.7.4 This sub-block MUST enable organizers, auditors, and administrators to query specific metrics, trends, and statistics from the logged data and get reports accordingly.

6.7.5 All logged messages should have information about their category, date-time and optionally location stamp, who from which entity is logging, etc.
...
6.8.1 This component MUST enable Building Block admins to configure rules for monitoring, detection, and escalation of any performance/security conditions to the administrator proactively. The exact parameters and statistical indicators may be decided at implementation time.
...
### **6.10. Pub/Sub Interface:**

6.10.1 These are dedicated API interfaces defined in the Pub/Sub (Information Mediator Building Block) and hence are not defined here.

6.10.2 This Block Should maintain the endpoint address of specific rooms dedicated to the scheduler Building Block to publish Alert notifications.

6.10.3 It MUST also use the logging sub-block to maintain the history of Pub/Sub transactions handled by the Scheduler Building Block.
...
6.11.2 It SHOULD send relevant information to the logging sub-block to maintain the history of all messages sent from this Building Block, which are useful for audit purposes.
...
6.13.2 All resources MUST have been registered in Scheduler and MUST have been affiliated with specific entities before they can be enrolled in different events.
...
6.13.5 The Scheduler MUST map a single appointment to a single resource or subscriber as a participant for a single event and be given a unique appointment ID.
...
6.13.7 The Scheduler MUST not allow duplicate entries of the same resource or subscriber for the same event.

6.13.8 Once a resource or a subscriber is registered in the scheduler (this is a unique ID independent of any event or appointment), the same ID is used as Participant Id in whichever appointments that the given resource/subscriber participates.
...
6.13.10 Participants of an event must send ids of the event and appointment in headers of their responses/updates sent to the scheduler through any channel (Information Mediator Building Block/PubSub/Messaging Building Block).
